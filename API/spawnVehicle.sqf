...
if (money >= _price) then {
	...
	player sidechat format["spawned a vehicle (Type: %1, Position: %2) Player: %3 Money: $%4",_vtype,_pos, _person, money];   
}else{
    player sidechat format["Cannot afford! Player: %1 Money: $%2", _person, money];
}